...
<?php
	...
	function generate_stats_tab() {
		...
		printf( '
			<div class="et_dashboard_stats_contents">
	 ...
	 * @return string
	 */
	function generate_all_time_stats() {
		$options_array = ET_Bloom::get_bloom_options();
	...
	function generate_optins_stats_table( $orderby = 'conversion_rate', $include_header = false ) {
		...
		if ( 0 < $optins_count ) {
			$output .= sprintf(
				...
				( 0 !== $total_impressions )
					? round( ( $total_conversions * 100 ) / $total_impressions, 1, PHP_ROUND_HALF_EVEN ) . '%'
					: '0%'
			);
			$output .= '</ul>';
		}
	...
	function generate_pages_stats() {
		...
		if ( ! empty( $con_by_pages ) ) {
			...
			foreach ( $rate_by_pages as $page_id => $rate ) {
				...
				$average_rate = 0 != $rates_count ? round( $page_rate / $rates_count, 1, PHP_ROUND_HALF_EVEN ) : 0;
	...
	function conversion_rate( $optin_id, $removed_flag = 0, $page_id = 'all' ) {
		...
		$conversion_rate_output = round( $conversion_rate, 1, PHP_ROUND_HALF_EVEN );
	...
	function calculate_growth_rate( $list_id ) {
		...
		if ( -1 === $oldest_record ) {
			...
			$weeks_count = round( ( $oldest_record ) / 7, 0, PHP_ROUND_HALF_EVEN );
			$weeks_count = 0 == $weeks_count ? 1 : $weeks_count;
			$growth_rate = round( $total_subscribers / $weeks_count, 0, PHP_ROUND_HALF_UP );
		}
	...
	function generate_stats_graph_output( $period, $day_or_month, $data ) {
		...
		if ( 0 < $bars_count ) {
			$per_day = round( $data['total_subscribers_' . $period] / $bars_count, 0, PHP_ROUND_HALF_UP );
		} else {
			$per_day = 0;
		}
	...
	function generate_optins_list( $options_array = array(), $status = 'active' ) {
		...
		if ( 'active' === $status && 0 < $optins_count ) {
			$output .= sprintf(
				...
				( 0 !== $total_impressions )
					? esc_html( round( ( $total_conversions * 100 ) / $total_impressions, 1, PHP_ROUND_HALF_EVEN ) . '%' )
					: '0%'
			);
		}
	...
	/**
	 * Sets "removed_flag" to 1 for the removed optins. We do not remove the optins from stats table to include them into overall stats.
	 */
	...
	function get_mailpoet_lists( $name ) {
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$table_name'" ) != $table_name ) {
			$error_message = __( 'MailPoet plugin is not installed', 'bloom' );
		} else {
			$error_message = 'success';
			$sql = "SELECT * FROM $table_name";
			$all_lists_array = $wpdb->get_results( $sql, ARRAY_A );

			if ( ! empty( $all_lists_array ) ) {
				foreach ( $all_lists_array as $list_details ) {
					$lists[$list_details['list_id']]['name'] = $list_details['name'];

					$sql_users = "SELECT COUNT(*) FROM $table_users WHERE list_id = %s";
					$sql_args = array(
						$list_details['list_id'],
					);

					$subscribers_count = $wpdb->get_var( $wpdb->prepare( $sql_users, $sql_args ) );
	...
	function subscribe_mailpoet( $list_id, $email, $name = '', $last_name = '' ) {
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$table_user'" ) != $table_user ) {
			$error_message = __( 'MailPoet plugin is not installed', 'bloom' );
		} else {
			...
			if ( 0 == $subscribers_count ) {
				...
				$sql_lists = "SELECT COUNT(*) FROM $table_user_lists WHERE list_id = %s";
				$sql_args_lists = array(
					$list_id
				);
				$lists_count = $wpdb->get_var( $wpdb->prepare( $sql_lists, $sql_args_lists ) );

				if ( 0 != $lists_count ) {
					// we need user ID to update lists table
					$sql_user = "SELECT * FROM $table_user WHERE email = %s";
					$sql_args_user = array(
						$email,
					);

					$added_user = $wpdb->get_results( $wpdb->prepare( $sql_user, $sql_args_user ), ARRAY_A );
					$current_user_id = $added_user[0]['user_id'];
		...
		$output = sprintf( '
			...
			( '' !== $formatted_title || '' !== $formatted_message )
				? sprintf(
					...
					stripslashes( html_entity_decode( $formatted_title ) ),
					stripslashes( html_entity_decode( $formatted_message ) )
				)
				: '',
			( 'below' == $details['image_orientation'] && 'widget' !== $details['optin_type'] ) || ( isset( $details['image_orientation_widget'] ) && 'below' == $details['image_orientation_widget'] && 'widget' == $details['optin_type'] )
				...
				: '', //#5
			'no_name' == $details['name_fields'] || ( ET_Bloom::is_only_name_support( $details['email_provider'] ) && '1' != $details['display_name'] )
				? ' et_bloom_1_field'
				: sprintf(
			...
			'custom_html' == $details['email_provider']
				? html_entity_decode( $details['custom_html'] )
				: sprintf( '
					%1$s
					...
					'' != $email_text ? esc_attr( $email_text ) : __( 'Email', 'bloom' ),
					'no_name' == $details['name_fields'] || ( ET_Bloom::is_only_name_support( $details['email_provider'] ) && '1' != $details['display_name'] )
						? ''
						: sprintf(
							...
							'first_last_name' == $details['name_fields']
								? sprintf(
									'%1$s',
									'' != $first_name_text
										? esc_attr( $first_name_text )
										: __( 'First Name', 'bloom' )
								)
								: '',
							( 'first_last_name' != $details['name_fields'] )
								? sprintf( '%1$s', '' != $single_name_text
									? esc_attr( $single_name_text )
									: __( 'Name', 'bloom' ) ) : '',
							'first_last_name' == $details['name_fields'] && ! ET_Bloom::is_only_name_support( $details['email_provider'] )
								? sprintf( '
									...
									'' != $last_name_text ? esc_attr( $last_name_text ) : __( 'Last Name', 'bloom' )
								)
								: ''
						),
					esc_attr( $optin_id ),
					...
					'' != $button_text ? esc_html( $button_text ) :  __( 'SUBSCRIBE!', 'bloom' ),
					isset( $details['button_text_color'] ) ? esc_attr( $details['button_text_color'] ) : '' // #10
				), //#9
			'' != $success_text
				? esc_html( $success_text )
				: __( 'You have Successfully Subscribed!', 'bloom' ), //#10
			$formatted_footer,
			'custom_html' == $details['email_provider']
				? sprintf(
	...
	function generate_inline_form( $optin_id, $details, $update_stats = true ) {
		...
		$output .= sprintf(
			'<div class="et_bloom_inline_form et_bloom_optin et_bloom_%1$s%9$s">
				%10$s
				<div class="et_bloom_form_container et_bloom_popup_container%3$s%4$s%5$s%6$s%7$s%8$s">
					%2$s
				</div>
			</div>',
			esc_attr( $optin_id ),
			...
			if ( 'accounts' != $id && 'db_version' != $id && isset( $single_optin['optin_type'] ) ) {
				if ( 'inactive' !== $single_optin['optin_status'] ) {
					...
					if ( ( ( 'flyin' == $single_optin['optin_type'] || 'pop_up' == $single_optin['optin_type'] || 'below_post' == $single_optin['optin_type'] ) && $this->check_applicability ( $id ) ) && ( isset( $single_optin['custom_css'] ) || isset( $single_optin['form_bg_color'] ) || isset( $single_optin['header_bg_color'] ) || isset( $single_optin['form_button_color'] ) || isset( $single_optin['border_color'] ) ) ) {
						...
						if ( isset( $single_optin['custom_css'] ) ) {
							$custom_css .= 	' ' . $single_optin['custom_css'];
						}
					}
