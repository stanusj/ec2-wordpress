...
class ET_Dashboard {
	...
	function dashboard_scripts( $hook ) {
		...
		wp_enqueue_script( 'chosen-js', ET_DASHBOARD_PLUGIN_URI . '/js/chosen/chosen.jquery.min.js', array( 'jquery' ), $this->class_version, true );
		wp_enqueue_style( 'chosen-css', ET_DASHBOARD_PLUGIN_URI . '/js/chosen/chosen.min.css', array(), $this->class_version );
						...
						foreach( $options_array as $option) {
							...
							switch( $option[ 'type' ] ) {
								...
								case 'pages_select' :
									...
									if ( 'page' == $option['post_type'] ) {
										$pages = get_pages();
									} else {
										$dashboard_post_types = ! empty( $dashboard_post_types ) ? $dashboard_post_types : array();
										foreach ( $dashboard_post_types as $post_type ) {
											if ( 'page' !== $post_type ) {
												...
												$posts_set = empty( $posts_set ) ? array() : $posts_set;
												$pages = array_merge( $pages, $posts_set );
											}
										}
									}
									...
									foreach ( $pages as $page ) {
										$page_options .= sprintf(
											...
											esc_attr( $page->ID ),
											selected( in_array( $page->ID, $current_option_value ), true, false ),
											'page' == $option['post_type']
												? esc_html( $page->post_title )
												: '[' . esc_html( $page->post_type ) . '] - ' . esc_html( $page->post_title )
										);
									}
									printf( '
										<li class="%4$s%6$s"%7$s%8$s>
											<select name="et_dashboard[%5$s][]" class="et_dashboard_select_pages" data-placeholder="%1$s"%3$s>
												%2$s
											</select>
										</li>',
										esc_attr( $option[ 'placeholder' ] ),
										$page_options,
										true === $option[ 'multiple' ] ? ' multiple' : '',
										false === $option[ 'multiple' ] ? 'et_dashboard_single_select' : 'et_dashboard_multiple_select',
										esc_attr( $current_option_name ), // #5
										isset( $option[ 'display_if' ] ) ? ' et_dashboard_hidden_option' : '',
										isset( $option[ 'display_if' ] ) ? ' data-condition="' . esc_attr( $option[ 'display_if' ] ) .  '"': '',
										isset( $option[ 'display_if' ] ) ? ' data-triggers_count="0"': '' // #8
									);
